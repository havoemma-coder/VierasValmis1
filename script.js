document.addEventListener('DOMContentLoaded', function() {
    ...
    function laskeJaNaytaHinta() {
        ...
        if (neliot > 0 && valittuPaketti && hinnat[valittuPaketti]) {
            ...
            laskettuHintaDiv.style.display = 'none';
        }
    }
    ...
    form.addEventListener('submit', async function(event) {
        ...
        // Näytä käyttäjälle, että lähetys on käynnissä
        ...
        try {
            // Lähetä tiedot serverless-funktiolle
            ...
            if (response.ok) {
                // Onnistui! Näytä kiitosviesti.
                document.querySelector('.tilaa-wrapper').style.display = 'none';
                kiitosViesti.style.display = 'block';
            } else {
                // Palvelimelta tuli virhe
                throw new Error('Palvelimella tapahtui virhe. Yritä myöhemmin uudelleen.');
            }
        } catch (error) {
            // Verkkovirhe tai muu tekninen ongelma
            alert(error.message);
